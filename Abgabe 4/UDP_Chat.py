        ...
        try:
            ...
            if c_address not in contacts.values() and line.lower().startswith("hallo"):
                with contact_added:  # Synchronisation starten
                    ...
                    print("Nachricht von unbekanntem Kontakt: Enter zum bestätigen")
            ...
            elif line == "Was ist deine IP-Adresse?":
                for ip, port in contacts.values():
                    if ip == c_address[0]:
                        send_lines(ip, port, get_local_ip())

            elif line == "Wie viel Uhr haben wir?":
                for ip, port in contacts.values():
                    if ip == c_address[0]:
                        send_lines(ip, port, f"Wir haben {datetime.now().strftime('%H:%M:%S')} Uhr.")

            elif line == "Welche Rechnernetze HA war das?":
                for ip, port in contacts.values():
                    if ip == c_address[0]:
                        send_lines(ip, port, "4. HA, Aufgabe 4")
...
def main(port):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        ...
        while True:
            ...
            user_input = input("Befehl eingeben ('send hello', 'send to', 'my address', 'my contacts', 'send all', 'register', 'stop'): ").strip()
            ...
            if command == "send hello":
                ...
                if len(contacts.keys()) != 0:
                    message = input("Was willst du allen Kontakten mitteilen? ")
                    for ip, port in contacts.values():
                        send_lines(s, ip, port, message)

            elif command == "register":
                name, ip, port = input("Gebe einen Namen , eine IP-Adresse ind eine Portnummer an, um sie den Kontakten hinzuzufügen: {name} {ip} {port}" ).strip().split()
...
if __name__ == '__main__':
    ...
    username = ""
    commands = ["send to", "send hello", "my address", "my contacts", "send all", "register"]
    contacts = {}
    start()
